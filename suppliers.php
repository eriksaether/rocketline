...
if (isset($_COOKIE['firstname']) && isset($_COOKIE['username']) ) {
  ...
  if (isset($_POST['businessname'])) {
    ...
    // Grab the profile data from the POST
    $businessname = cleanMyPost($_POST['businessname']);
    ...
    if (!empty($businessname) && !empty($contactname) && !empty($telephone) ) {

      // Make sure someone isn't already registered using this username
      $query = "SELECT * FROM suppliers WHERE businessname = '$businessname'";
      $data = mysqli_query($connex, $query);
      if (mysqli_num_rows($data) == 0) {
        // The username is unique, so insert the data into the database
        $query = "INSERT INTO suppliers (businessname, contactname, telephone, emailaddr, vatnumber, address, postcode, commid, joindate) VALUES ('$businessname', '$contactname','$telephone', '$emailaddr', '$vatnumber', '$address', '$postcode', $useride, NOW())";
      ...
      echo '<p class="mirror">You must enter all of the data.'.$businessname.$contactname.$telephone.'</p>';
    }
  }

       //update database with supplier line correction
        if (isset($_POST['suppliername']) && isset($usernaam)) {

            $idedit= $_POST["thumbit"];
            $suppliername= cleanMyPost($_POST["suppliername"]);
            $contactname= cleanMyPost($_POST["contactname"]);
            $telephone = cleanMyPost($_POST['telephone']);
            $emailaddr = cleanMyPost($_POST['emailaddr']);
            $vatnumber = cleanMyPost($_POST['vatnumber']);
            ...
            $queryvee = "UPDATE suppliers SET businessname = '$suppliername', contactname = '$contactname', telephone = '$telephone', emailaddr = '$emailaddr', vatnumber = '$vatnumber', address = '$address', postcode = '$postcode' WHERE supplierid = '$idedit' ";
            mysqli_query($connex, $queryvee);
            echo "supplier:".$idedit." updated.";
        }        
        ...
        if (!empty($_POST["iddelete"])) {
          ...
          if ($idnum > 0) { 
            ...
            $sqldel = "DELETE FROM suppliers WHERE supplierid=$idnum";
 ...
 if (isset($_POST['lineone'])) {
     ...
     $postcode = cleanMyPost($_POST['postcode']);
     $supplierid = cleanMyPost($_POST['supplierid']);      
     $row_cnt=0;
     
     $queryaddress = "SELECT * FROM addresses WHERE supplierid = '$supplierid' AND type = '$typeaddress'";      
     ...
     if(isset($lineone)) { 
         if ($row_cnt<0.1) {
           $qaddress =  "INSERT INTO addresses (type, name, lineone, linetwo, linethree, postcode, supplierid) VALUES ('$typeaddress', '$nameinfo', '$lineone', '$linetwo', '$linethree', '$postcode', '$supplierid')";
           mysqli_query($connex, $qaddress);
           echo"Address added.";
         }     
         elseif ($row_cnt>0.1)  {
           $qaddress = "UPDATE addresses SET  type = '$typeaddress', name = '$nameinfo', lineone = '$lineone', linetwo = '$linetwo', linethree = '$linethree', postcode = '$postcode' WHERE supplierid = '$supplierid' AND type = '$typeaddress'";
           mysqli_query($connex, $qaddress);
           echo"Address edited.";
         } 
 ...
 if (isset($_POST['idviewgo'])) {
   ...
   $supplierid = $_POST['idviewgo'];
   
   //grab item info
   $queryaddr = "SELECT * FROM addresses WHERE supplierid = '$supplierid'";    
   ...
   if($row_cnt=0) {
    ...
    echo "<form class='displaybox' id='displaybox' action='suppliers.php' method='post'><b><u>Address</u><br><br></b>";
    ...
    echo "<input hidden type='number' name='supplierid' value='".$supplierid."'>"; 
  ...
  echo "<h4>Supplier Information</h4>";
  ...
  //check privileges
  if($up>0) {
    $sqlq = "SELECT * FROM suppliers";
    echo"<span style='float: right; color: red;'>Admin view</span>";
    //check if searching
    if($searcher > 0) {
        $sqlq = "SELECT * FROM suppliers WHERE orderstatus = '$searchstatus' ";
    }
  }
  else {  
    $sqlq = "SELECT * FROM suppliers WHERE commid = '$useride' ";
    //check if searching
    if($searcher > 0) {
        $sqlq = "SELECT * FROM suppliers WHERE commid = '$useride'  AND orderstatus = '$searchstatus' ";
    } 
  }
  $resultq = $connex->query($sqlq);
  
  echo "<table width='100%' style='  box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);'> ";
  echo "<tr  class='top'><td width='20px' ></td><td width='200px'>Business name</td><td>&nbsp;Ownid&nbsp;</td><td>VAT number</td><td >Contact person</td><td>Telephone</td><td>Email</td><td >Addr.&nbsp;</td><td ></td><td class='statusinfo' width='40px'>Status</td><td></td></tr>";
  ...
  while($row = $resultq->fetch_assoc()) {
         ...
         if($up ==1) {
            echo "<td class='redx' onclick='myDelete(".$supplierid[$counter].");'>x</td>";
         }
         else{
            echo "<td></td>";
         }
         echo "<td id='suppliernamo".$counter."'>".$businessname[$counter]."</td>";
         echo "<td id='supplierido".$counter."'>".$ownerid[$counter]."</td>";
         ...
         echo "<td id='addresso".$counter."' onclick='myView(".$supplierid[$counter].");' class='glyphicon glyphicon-home orange' style='cursor:pointer; color: gray;'></td>";
         echo "<td id='postcodo".$counter."'></td>";
         echo "<td id='statuso".$counter."'>".$status[$counter]."</td>";
         echo "<td style='cursor:pointer; color: gray;' class='glyphicon glyphicon-pencil' onclick='laGoon(".$counter.",".$supplierid[$counter].");'></td>";
         echo "</tr>";
         
  }
  $connex->close();
  if($counter <1) {echo "<tr><td></td><td>You have no suppliers<td></tr>";}

  //ready new input
        echo "<tr><form action='suppliers.php' id='myFormAdder' method='post'><td></td>";
        echo "<td><input type='text' class='busname' name='businessname'></td>";
        echo "<td></td>";
...
<form action='suppliers.php' id='myFormDel' method='post'>
  <input type='number' id='iddelete' name='iddelete' style='width: 40px' hidden>
</form>

<form action='suppliers.php' id='myFormEdit' method='post' hidden>
...
<dir id="tester"></dir>
  ...
  <form action='suppliers.php' id='myFormView' method='post'>
...
function laGoon(abc, littlejohn){
    var suppliernamo = "suppliernamo"+abc;
    ...
    var suppliername =  document.getElementById(suppliernamo).innerHTML;    
    ...
    document.getElementById(urhere).innerHTML = "<tr><td><input type='number' name='idthumb'  value='"+urhere+"' hidden></td>"+"<td><input type='text' id='suppliernameso' class='busname' value='"+suppliername+"'></td><td><input type='number' id='supplieridso' name='thumbit' value='"+littlejohn+"' hidden>"+littlejohn+"</td><td><input type='text' name='vatnumber' id='vatnumberso' value='"+vatnumber+"'></td><td><input type='text' id='contactnameso' name='contactname' value='"+contactname+"'></td><td><input type='text' id='telephoneso' name='telephone' value='"+telephone+"'></td><td><input type='text' id='emailaddrso' name='emailaddr' value='"+emailaddr+"'></td><td><input type='text' id='addressso' class='address' name='address' value='"+address+"'></td><td><input type='text' id='postcodo' name='postcode' class='postcode' value='"+postcode+"'></td><td><input type='text' id='statusso' name='status' value='"+status+"'></td><td><button class='glyphicon glyphicon-ok smallgreen' onclick='addSubmit();'></button></td></tr>"; 
...
function addSubmit() {    
    ...
    if(vatnum.length == 10 || vatnum == 0){
      ...
      document.getElementById("thumbit").value = document.getElementById("supplieridso").value;
      document.getElementById("suppliername").value = document.getElementById("suppliernameso").value;
      ...
      document.getElementById("address").value = document.getElementById("addressso").value;
      document.getElementById("postcode").value = document.getElementById("postcodo").value;

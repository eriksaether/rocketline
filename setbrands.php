...
if (isset($_COOKIE['firstname']) && isset($_COOKIE['username']) ) {
  ...
  if (!empty($_POST["iddelete"])) {
    ...
    if ($idnum > 0) { 
      ...
      $sqldel = "DELETE FROM brandnames WHERE brandid=$idnum";
      if (mysqli_query($connex, $sqldel)) {
        echo "brandnames deleted successfully. ";
      } else {
        echo "Error deleting record: " . mysqli_error($connex);
      }
      $sqldel2 = "UPDATE `stocktypes` SET brandid = 0 WHERE brandid = $idnum";
      if (mysqli_query($connex, $sqldel2)) {
        echo "brandnames deleted successfully from stock items as well. ";
      } else {
        echo "Error deleting record: " . mysqli_error($connex);
      }
  ...
  echo "<h4>Brand Information</h4>";
  echo "<table width='100%' style='  box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);'> ";
  echo "<tr  class='top'><td width='20px' ></td><td width='240px'>brand name</td><td></td></tr>";
  ...
  while($row = $resultq->fetch_assoc()) {
         ...
         $counter = $counter+1;
         $brandname[$counter] = $row["brandname"];
         $brandid[$counter] = $row["brandid"];                       
      
         echo "<tr style='border-bottom: solid 1px lightgray;' id='therow".$counter."'>";
         if($up >-1) {
            echo "<td class='redx' onclick='myDelete(".$brandid[$counter].");'>x</td>";
         }
         else{
            echo "<td></td>";
         }
         echo "<td id='clientnamo".$counter."'>".$brandname[$counter]."</td>";
  ...
  echo "<td><input type='text' class='busname' name='brandname' value='";
...
<form action='setbrands.php' id='myFormDel' method='post'>
  <input type='number' id='iddelete' name='iddelete' style='width: 40px' hidden>
</form>

<form action='setbrands.php' id='myFormEdit' method='post' hidden>
...
<dir id="tester"></dir>
  ...
  <form action='setbrands.php' id='myFormView' method='post'>

...
<form class="navi" action='brands.php' method="post">Search Brands<br>
  ...
  <button class="inbutton">go</button>
   ...
   <!--- controller/selector -->
    ...
    <form action='' id='myTaskView' method='post' class="graydisplay">
      ...
      <form action='' id='myTaskView' method='post' class="graydisplay">
      <div>
        <?php if (isset($useride)) {echo"<span class='red'><a href='index.php?idviewbrand=All'>All Brands</a></span>"; }
        else{ echo"<span class='red'><a href='index.php?idviewbrand=All'>All Brands</a></span>"; } ?>
      </div>
      <br>    
    </form>
<br>
...
echo "<div class='blow'>" ;
  ...
  if($searcher==3) {
       $user_search = " WHERE brandname LIKE '%{$searchcritter}%'";
  }
  elseif($searcher==2) {
  ...
  elseif ($searcher==0) {
       $user_search = " ORDER BY brandname DESC";
  }  
  ...
  while ($row = mysqli_fetch_array($result)) {   
      ...
      echo "<div class='col-4'>";      
        ...
        if (!empty($row['brandpic'])) {
         echo "<div class='trimbox'><img class='col-12 picboxes' src='" . MM_UPLOADPATH . $row['brandpic'] .
        "' alt='Stock Picture' onclick='myViewee(".$row['brandid'].");'></div>";
        }
        else {
        ...
        "' alt='Stock Picture' onclick='myViewee(".$row['brandid'].");'></div>";
          ...
          echo "<div class='boxtext' onclick='myViewGo(".$row["brandid"].");'><span class='left'><b>".$row['brandname']."</b></span></div>"; 
        echo"</div>"; 
      echo"</div>"; 
...
?>
  ...
  <form action='brands.php' id='myFormViewGo' method='post'>
    <input type='number' id='idviewgo' name='idviewgo' style='width: 40px' hidden>
  </form>
   <form action='index.php' id='myFormViewer' method='get'>
    <input type='number' id='idviewfocus' name='idviewbrand' style='width: 40px' hidden>
  </form>
  <form action='brands.php' id='myFormAdd' method='post'>

...
if (isset($_COOKIE['firstname']) && isset($_COOKIE['username']) ) {
  ...
  if (!empty($_POST["iddelete"])) {
    ...
    if ($idnum > 0) { 
      ...
      $sqldel = "DELETE FROM categories WHERE catid=$idnum";
  ...
  if (!empty($_POST["idedit"])) {
         ...
         $qcat = "UPDATE categories SET name = '$catname' WHERE catid = '$idedit'";
  ...
  if($up>0) {
    ...
    $sqlq = "SELECT * FROM categories WHERE catid =0";
    echo "You are not authorised to edit this.";
  }
  ...
  echo "<tr  class='top'><td width='20px' ></td><td width='240px'>category name</td><td></td><td></td></tr>";

  $catname = array();
 
  while($rowcat = $resultq->fetch_assoc()) {

         $counter = $counter+1;
         $catname[$counter] = $rowcat["name"];
         $catid[$counter] = $rowcat["catid"];                       
      
         echo "<tr style='border-bottom: solid 1px lightgray;' id='therow".$counter."'>";
         if($up >-1) {
            echo "<td class='redx' onclick='myDelete(".$catid[$counter].");'>x</td>";            
         }
         else{
            echo "<td></td>";
         }
         echo "<td id='clientnamo".$counter."'>".$catname[$counter]."</td>";
         if($up >-1) {
            echo "<td style='cursor: pointer;' onclick='laGoon(".$counter.",".$catid[$counter].");'><button>edit</button></td>";
         }
         echo "</tr>";
  ...
  echo "<td><input type='text' class='busname' name='catname' value='";
...
<form action='categories.php' id='myFormDel' method='post'>
  <input type='number' id='iddelete' name='iddelete' style='width: 40px' hidden>
</form>

<form action='categories.php' id='myFormEdit' method='post' hidden>
...
<dir id="tester"></dir>
  ...
  <form action='categories.php' id='myFormView' method='post'>
...
function addSubmit(littlejohn) {    
      ...
      document.getElementById("catname").value = document.getElementById("clientnameso").value;              

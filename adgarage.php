...
?>
  ...
  <nav>
  <form action="adgarage.php" method="get"  class="searchbox">
  <span class="space">  
    <span class="leftpad"><b>Find your item:</b></span><br />
   ...
   <form class="graydisplay">
      <div >
        <span class="red" onclick="showEventer(1);" >Load Event</span>   
      </div>
      <br>
  </form>
  <form action='' id='myTaskView' method='post' class="graydisplay">
      <div>
        <span class="red" onclick="showRecorder(1);" >Load lot</span>   
      </div>
      <br>    
  </form>
   <form action='' id='loadfeatures' method='post' class="graydisplay">
      <div>
  ...
  <form class="graydisplay">
      <div >
        <span><a href="adgarage.php" class="red">Refresh</a></span>
      </div>
      <br>
  </form>
...
if (isset($_COOKIE['firstname']) && isset($_COOKIE['username']) ) {
  ...
  if (isset($_POST['eventname'])) {
    ...
    $eventname = cleanMyPost($_POST['eventname']);    
    $expirydate = $_POST['expirydate'];
    
    $eventide = 0;
    require_once("loadevent.php");
  }
  ...
  while ($row = mysqli_fetch_array($result)) {
    ...
    echo '<td valign="top" style="cursor:pointer;" width="50%" onclick="myView('.$row['stockid'].');">' . substr($row['description'], 0, 100) . '...</td>';
    ...
    echo '<td valign="top" width="12%">' . substr($row['creationdate'], 0, 10) . '</td>';
       ...
       echo '<td  width="30px" style="cursor:pointer; " class="glyphicon glyphicon-pencil" onclick="myEdit('.$row['stockid'].');"></td>';
       echo '<td  width="30px" style="cursor:pointer;" class="glyphicon glyphicon-equalizer" onclick="myAttrEdit('.$row['stockid'].');"></td>';
...
<div class="recorder" id="recorder">
    <form enctype="multipart/form-data" id="myFormLoader" method="post" action="adgarage.php" > 
      
      <input type="hidden" name="stockide" value="<?php if (!empty($stockide)) echo $stockide; ?>">

      <label>Lot Name:</label><input type="text" id="workoutblue" class="workoutblue" required name="stockname" value="<?php if (!empty($stockname)) echo $stockname; ?>"><br>
      <label>Stockcode:</label><input type="text" id="" class="" name="stockcode" value="<?php if (!empty($stockcode)) echo $stockcode; ?>"><br>
      <label for="lineone">Stock Description</label> 
        <textarea name="stockdescribe" form="myFormLoader"  rows="2" cols="55" required ><?php if (!empty($stockdescribe)) echo $stockdescribe; ?></textarea>

      <input name="tool" value="recorder" hidden><br>
      
      <label>Category:</label><input type="text" required name="category" value="<?php if (!empty($stockcategory)) echo $stockcategory; ?>">      
     
      <label>Subcategory:</label><input type="text" name="subcategory" value="<?php if (!empty($stocksubcategory)) echo $stocksubcategory; ?>"><br>
      
      
      <label>Reserve price:</label><input type="text" required name="costprice" value="<?php if (!empty($costprice)) echo $costprice; ?>"><br>
      ...
      <label>Sale event:</label>
      <select name="auctionid" >
        <option value=0>Not needed</option>
      <?php 
        $sqlbrands = "SELECT * FROM garagesales ";  
        $resultbands = $dbc->query($sqlbrands); 
        
        while($rowsbats = $resultbands->fetch_assoc()) {           
            if($rowsbats['eventid'] == $brandid) {

              echo"<option value='".$rowsbats['eventid']."' selected>".$rowsbats['eventname']."</option>";
            }
            else {
              echo"<option value='".$rowsbats['eventid']."'>".$rowsbats['eventname']."</option>";
            }         
        }
        
      ?>
      </select>      
      <label>Auction Yes?</label>
...
<form id="brander" enctype="multipart/form-data" class="brander" action="" method="post">
      ...
      <input type="button" id="exit" name="" class="exit" value="X" onclick="sBrander = 0; showEventer(0);">
</form>
